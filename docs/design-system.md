# IELTS Arena — Visual Design System v1.0
...
Two things make IELTS Arena different from a generic dashboard, and the whole system is built around them:

1. **The band score** — IELTS grades everything on a 0–9 scale in 0.5 steps. That's not just a number, it's the app's native unit of measurement, so it gets its own recurring visual device (the **Band Meter**, §4) instead of a generic progress ring or percentage bar.
2. **Skill modules** — Reading, Writing, Listening (and later Speaking) are structurally separate but share one framework (per the PRD's "skill module" scoping principle). Each skill gets its own identity color, but they all sit inside the same neutral chrome, so the app reads as one product with four instruments, not four different apps stapled together.

Everything below derives from those two facts.
...
Each skill module gets one accent, used for its tag, its icon tint, its active nav state, and light fills behind its own content. Never use a skill's color for chrome shared across skills (buttons, global nav background, etc.) — that's what keeps four modules feeling like one app.

| Skill | Token | Hex | Fill (10%) |
|---|---|---|---|
| Reading | `reading-600` | `#1C7A6C` | `#E4F3F0` |
| Writing | `writing-600` | `#6D4FC9` | `#EEEAFB` |
| Listening | `listening-600` | `#2C6FD1` | `#E7EFFC` |
| Speaking (coming soon) | `speaking-600` | `#B8623D` | `#F5EAE3` |
...
| `display-sm` | 28px / 1.2 | 600, Fraunces | Band-score reveal on a graded result |
| `h1` | 24px / 1.3 | 600, Inter | Page title |
| `h2` | 19px / 1.35 | 600, Inter | Section header |
| `h3` | 16px / 1.4 | 600, Inter | Card header, question group title |
...
- **Radius:** 8px for buttons/inputs/tags, 12px for cards, full-circle for avatar frames. Don't mix a third radius value in.
- **Elevation:** flat by default. Cards use a 1px hairline border, not a shadow. Reserve shadow for things that float above content: modals (`0 8px 24px rgba(22,35,63,0.12)`), dropdowns. A dashboard screen should have at most one floating element at a time.

---

## 4. Signature component — the Band Meter
...
- **Skill tag** — pill, 8px radius, skill's 10%-fill background, skill's `-600` text, `label` type. e.g. "READING" in teal.
...
- **Question card** — `surface-0`, 12px radius, hairline border, 24px padding, skill tag top-left, question text in `body`.
...
- **Buttons** — primary: `ink-900` fill, white text. Secondary: `surface-0` fill, hairline border, `ink-900` text. Only one primary button visible per screen. Skill colors never appear on buttons — they're for identifying content, not actions.
...
3. **Build the component library before the pages.** Ask for Button, Tag, StatusBadge, BandMeter, Card, AvatarFrame as isolated components on one showcase/storybook-style page first. Only once those exist do you ask for "the submission history page" — composed from those pieces, not styled fresh.
...
5. **Give the assistant the constraint, not just the goal.** Instead of "make this look nicer," say "match §4 Band Meter spec exactly: 18 segments, 4px gap, gold-600 fill" — cite the section. Specific constraints produce specific, checkable output.
...
8. **Keep a running notes file of decisions and rejected directions** (e.g. `frontend-notes.md`) — "tried a circular progress ring for band score, rejected in favor of the Band Meter because it ties to the actual 0.5-step grading scale." This stops future sessions from re-litigating settled choices.
